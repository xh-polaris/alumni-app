/**
 * 中国行政区划数据（随包发布，不走网络请求）。
 *
 * 数据口径：
 * - 省级：全部 34 个省级行政区，使用 GB/T 2260 官方 6 位代码；
 * - 市级：prefecture-level 行政区（地级市 / 地区 / 自治州 / 盟，直辖市为市辖区）。
 *   代码同样使用 GB/T 2260 官方 6 位代码。
 * - 名称统一使用简称（`浙江省`、`杭州市`、`上海市`），不带「省/市」以外的后缀。
 * - 直辖市的市级列表同时包含「全市」条目（`310100 上海市`）与市辖区，
 *   这样迁移产生的历史数据（cityCode=310100）仍能反查到名称。
 *
 * 取舍原则：无法确认官方代码的条目直接省略（宁缺勿错），
 * 省略清单记录在 docs/superpowers/plans/2026-06-15-chapter-refactor-app.md。
 */
...
export const PROVINCES: ProvinceOption[] = [
  ...
  // 港澳没有 GB/T 2260 意义上的地级市代码，这里使用通行的大区划分与占位代码，
  // 仅用于让二级选择器可用；如需精确到区，建议由服务端补充数据。
  {
    code: "810000",
    name: "香港特别行政区",
    cities: [
...
const PROVINCE_INDEX = new Map<string, ProvinceOption>(PROVINCES.map((item) => [item.code, item]));

/**
 * 通过代码反查省市名称。任一层级缺失时回退为空字符串，页面自行决定占位文案。
 */
export function findRegion(provinceCode: string, cityCode: string): { provinceName: string; cityName: string } {
  const province = provinceCode ? PROVINCE_INDEX.get(provinceCode) : undefined;
  const city = province && cityCode ? province.cities.find((item) => item.code === cityCode) : undefined;
  return {
 ...
 * 按代码取省级条目，供省市联动选择使用。
 */
export function findProvince(provinceCode: string): ProvinceOption | undefined {
  return provinceCode ? PROVINCE_INDEX.get(provinceCode) : undefined;
}
